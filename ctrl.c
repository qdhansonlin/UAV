...
void CTRL_2(float T)
{
	ctrl_2.PID[PIDROLL].kd=0.3;//手动PID参数
	ctrl_2.PID[PIDPITCH].kd=0.3;
	ctrl_2.PID[PIDYAW].kd=0.1;
	
	ctrl_2.PID[PIDROLL].ki=0.05;
	ctrl_2.PID[PIDPITCH].ki=0.05;
	ctrl_2.PID[PIDYAW].ki=0.05;
	
	ctrl_2.PID[PIDROLL].kp=0.5;
	ctrl_2.PID[PIDPITCH].kp=0.5;
	ctrl_2.PID[PIDYAW].kp=0.2;
...
//	except_A.y  = MAX_CTRL_ANGLE  *( my_deathzoom( (-CH_filter[PIT]) ,30 )/500.0f );  //30
	//添加了位置控制 by WY
	except_A.x  = expect_angle_roll+ MAX_CTRL_ANGLE  *( my_deathzoom( ( CH_filter[ROL]) ,30 )/500.0f );   //要看这两个值适不适合相加，需不需要标度统一
	except_A.y  = expect_angle_pitch + MAX_CTRL_ANGLE  *( my_deathzoom( (-CH_filter[PIT]) ,30 )/500.0f );  //30
	if( Thr_Low == 0 )
	{
...
{
	ctrl_1.PID[PIDROLL].kd=2.0;//手动PID参数
	ctrl_1.PID[PIDPITCH].kd=2.0;
	ctrl_1.PID[PIDYAW].kd=1.0;
	
	ctrl_1.PID[PIDROLL].ki=0.1;
	ctrl_1.PID[PIDPITCH].ki=0.1;
	ctrl_1.PID[PIDYAW].ki=1.0;
	
	ctrl_1.PID[PIDROLL].kp=0.8;
	ctrl_1.PID[PIDPITCH].kp=0.8;
	ctrl_1.PID[PIDYAW].kp=1.2;
