...
{
    /* 确定位置信息后进入功能 */
    if(gpsx.fixmode >= 2 && CH_filter[AUX2] > -200)// 这里为什么不直接用设置好的mode作条件
    {
        /* 当前位置经度赋值 单位 放大率10E5 */
        ...
        t_latitude=(int)gpsx.latitude;
			  ...
			  //对GPS出现的忽而跳0的问题，进行滤波，把误差数据都去掉
			  if(t_longitude == 0 )
					 t_longitude = d_longitude;			
				if(t_latitude == 0)
					 t_latitude = d_latitude;
					
        /* 当前位置经度中位数滤波 单位 放大率10E5 */
        t_longitude = Moving_Median_int(0,10,t_longitude);
        /* 当前位置纬度中位数滤波 单位 放大率10E5 */
        t_latitude = Moving_Median_int(1,10,t_latitude);
        ...
        /* 记录为上次数据 */
        d_longitude = t_longitude;  //对d_longgitude的处理
        ...
        */
        ...
        /* 飞行器向飞行器的前方速度 单位毫米每秒 */
        Speed_Front = north_speed * arm_cos_f32(Yaw*3.14159f/180.0f) //要确定这个yaw值对不对
                      - west_speed * arm_sin_f32(Yaw*3.14159f/180.0f);

        /* 飞行器向左速度，单位毫米每秒 */
        Speed_Left = west_speed * arm_cos_f32(Yaw*3.14159f/180.0f)
                     + north_speed * arm_sin_f32(Yaw*3.14159f/180.0f);
        ...
        /* 飞行器位置向飞行器前方偏移量 */ 
        Dist_Front = north_dist * arm_cos_f32(Yaw*3.14159f/180.0f)
                     - west_dist * arm_sin_f32(Yaw*3.14159f/180.0f);

        /* 飞行器位置向飞行器左方偏移量 */
        Dist_Left = west_dist * arm_cos_f32(Yaw*3.14159f/180.0f)
                    + north_dist * arm_sin_f32(Yaw*3.14159f/180.0f);
        ...
        if(position_ctrl_mode==3)
        {
            ...
            expect_speed_Front_P=pid_setup.groups.ctrl4.kp *expect_dist_Front;    //这些PID值根本就没赋值，没进来。可以还像ctrl.c一样在前面赋值
        ...
        {
            /* 记录经纬度 */
            STOP_longitude = t_longitude;//有在其它语句中使用
            STOP_latitude = t_latitude;
            STOP_ewhemi = gpsx.ewhemi;	//东经/西经,E:东经;W:西经，没有在其他语句中使用
