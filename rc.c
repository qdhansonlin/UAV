...
//todo: 悬停模式切换
		...
		switch(position_ctrl_mode)
			{
		     case 1:{
				 if(gpsx.fixmode >= 2)                                      //判断GPS是否正常接收信号
							{
								position_ctrl_mode = 2;                             //进入到正常位置控制部分
								position_record ();                                 //执行返航程序中位置记录部分，判断当前位置是否需要记录
										
					}
				  else 	
				        return_mode = 0;                                    //若GPS无正常接收信息，返回一个返航标志位
				}
		 
		     case 2:
				 {
					 if(gpsx.fixmode >= 2 && CH_filter[AUX2] > -200)         //判断GPS是否正常接收信号且进入到位置控制模式
								{
									if(STOP_longitude == 0 || STOP_latitude == 0)    //若刚进入到位置控制模式，即定点位置有坐标为0
										{
											STOP_longitude = (int)gpsx.longitude;        //把当前定位坐标记录下来
											STOP_latitude  = (int)gpsx.latitude;
										}
									else                                             //若早已进入到位置控制模式
										{
											if(CH_filter[AUX2] > 200 )                   //若进入到返航模式
												{
													return_mode = 1;                         //返航标志位进1
													position_return ();                      //执行返航程序
												}
											else                                         //若未进入到返航模式中
												{
													position_record ();                      //执行返航程序中位置记录部分，判断当前位置是否需要记录
									...
									position_ctrl_mode = 2;                          //保持飞行器处于返航模式下
								}
							else{ 
								position_ctrl_mode = 1;                            //若GPS未正常接收或者每进入到位置控制模式，控制模式置1
								STOP_longitude = 0;                                //返航位置坐标清零，方便下次进入定位模式使用
					    	STOP_latitude  = 0;
				        return_mode = 0;                                   //返航标志位置0
				 }
...
//	 
//	switch(position_ctrl_mode){
//		 case 1:{if(gpsx.fixmode >= 2){
//			          STOP_longitude = (int)gpsx.longitude;
//	              STOP_latitude  = (int)gpsx.latitude;
//			          if(CH_filter[AUX2] > 0){
//				           position_ctrl_mode = 2;}
//                else 	position_ctrl_mode = 1;	} 
//		          else 	position_ctrl_mode = 1;}
//		 
//							
//		 case 2:{if(gpsx.fixmode >= 2){
//			          position_ctrl_mode = 3;}
//		         else position_ctrl_mode = 1;}
//		 
//		 case 3:{if(gpsx.fixmode >= 2){
//			          STOP_longitude = (int)gpsx.longitude;
//	              STOP_latitude  = (int)gpsx.latitude;
//			          if(CH_filter[AUX2] > 0){
//									position_ctrl_mode = 3;}
